...
static void process_token(yyscan_t scanner, struct rules_state *state, struct rule_pair *pair)
{
	...
	switch (pair->key) {
		...
		case KEY_LABEL:
			...
			if (state->warning[W_DUPLICATE_LABELS]) {
				list_for_each_entry(label, state->labels, list) {
					if (pair->rule->file != label->rule->file)
						continue;

					if (strcmp(pair->value->string, label->name->string))
						continue;

					warnx("%s:%d: LABEL=\"%s\" value already exists. The first one was: %s:%d [-W%s]",
						state->cur_file->name,
						yyget_lineno(scanner),
						pair->value->string,
						label->rule->file->name,
						label->rule->line_nr,
						warning_str[W_DUPLICATE_LABELS]);
...
int rules_readdir(const char *dir, struct rules_state *state)
{
	...
	while ((ent = readdir(d)) != NULL) {
		...
		if (asprintf(&filename, "%s/%s", dir, ent->d_name) < 0) {
			warn("asprintf");
			continue;
		}
