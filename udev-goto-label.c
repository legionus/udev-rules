...
void check_goto_label(struct rules_state *state)
{
	...
	list_for_each_entry(r_goto, state->gotos, list) {
		...
		list_for_each_entry(r_label, state->labels, list) {
			...
			if (state->warning[W_LABEL_BEFORE_GOTO] &&
			    r_goto->rule->file == r_label->rule->file &&
			    r_goto->rule->global_order > r_label->rule->global_order) {
				warnx("%s:%d: LABEL=\"%s\" is before GOTO. The previous one was: %s:%d [-W%s]",
					r_goto->rule->file->name,
					r_goto->rule->line_nr,
					r_label->name->string,
					r_label->rule->file->name,
					r_label->rule->line_nr,
					warning_str[W_LABEL_BEFORE_GOTO]);
		...
		if (!found) {
			warnx("%s:%d: GOTO=\"%s\" has no matching label.",
				r_goto->rule->file->name,
				r_goto->rule->line_nr,
				r_goto->name->string);
			warning_update_retcode(state);
